...
export default async function RegionsIndexPage() {
  const items: RegionPressure[] = await getHeatmap();
    ...
    <main className="flex flex-1 flex-col gap-4 p-4 lg:p-6">
      <section className="flex flex-col gap-3 rounded-xl border border-border bg-card p-4">
        <header className="flex items-baseline justify-between gap-2">
          <div>
            <h2 className="text-sm font-semibold">Regiões administrativas</h2>
            <p className="text-[11px] text-muted-foreground">
              Drill-down por RA · score combinado de pressão · janela 24h
            </p>
          </div>
            ...
            return (
              <li key={r.raId}>
                <Link
                  ...
                  className="grid grid-cols-[1fr_auto_auto] items-center gap-3 rounded-md px-1 py-1.5 hover:bg-muted/40"
                >
